...
export const UsersList: React.FC<Props> = ({ project, group }) => {
  ...
  const findUser = async () => {
    ...
    setText('');
    setShowSuggestions(false); // Oculta as sugestões ao clicar no botão de busca
  };
  ...
  const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setShowSuggestions(true); // Mostra as sugestões ao digitar no campo de busca
  };

  const handleUserSelect = () => {
    setShowSuggestions(false); // Oculta as sugestões ao clicar em um usuário da lista
  };
  ...
  const addButton = (
    <button
      ...
      type="button"
      onClick={() => addUser(newUser)} // Corrigido para usar uma função de callback
      style={{
      ...
      <div className="bg-[#F2F2F2] dark:bg-[#333] w-80 md:w-96 py-8 lg:py-12 relative">
        <div className="flex flex-col gap-12 justify-between">
          <div>
            ...
            >
              <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 50 50" fill="currentColor" className="text-primary dark:text-secondary stroke-none">
