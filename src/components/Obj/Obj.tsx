...
export const Obj = ({ objs = [], max, functionObj }: Props) => {
    ...
    const classes = `rounded-full relative xl:w-8 xl:h-8 lg:w-6 lg:h-6 md:w-4 md:h-4 w-3 h-3 xl:-mr-4 lg:-mr-3 md:-mr-2 -mr-1 text-[10px] md:text-[14px] lg:text-[18px]  xl:text-[32px]
                ...
                objs.map((o, index) => {
                    if (index == max+1 && !isHovering) {
                        ...
                        if(o instanceof TaskPage){
                            ...
                            return <div className={classes} key={index} title={o.name ?? "Sem Nome"}> {o.picture && <Image fill alt={o.name ?? "Sem Nome"} src={archiveToSrc(o.picture)} />}</div>
                        }else{
                            return <div className={classes} key={index} title={o.name ?? "Sem Nome"}>{o.picture && <Image fill alt={o.name ?? "Sem Nome"} src={archiveToSrc(o.picture)} />}</div>
                        }
                    }
