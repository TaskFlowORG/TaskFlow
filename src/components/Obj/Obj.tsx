        ...
        <div className="flex justify-center z-20 relative" onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)}>
            {
                objs.map(o => {
                    if (objs.indexOf(o) == max && !isHovering) {
                        return <div className="rounded-full absolute w-8 h-8 bg-primary dark:bg-secondary text-white overflow-clip flex shadow-blur-10 items-center justify-center" key={o.id}>+</div>
                        ...
                        if(o instanceof TaskCanvas){
                            ...
                            return <div className="rounded-full absolute w-8 h-8 bg-primary dark:bg-secondary text-white overflow-clip flex shadow-blur-10 items-center justify-center" key={o.id} title={o.name ?? "Sem Nome"}> {o.picture && <img src={o.picture} />}</div>
                        }
                    }
                })
            }
