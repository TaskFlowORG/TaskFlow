...
export const SVGInitialPage = () => {
  ...
  useEffect(() => {
    if (theme == "light") {
      setPrimary(getComputedStyle(document.documentElement).getPropertyValue('--primary-color'));
      setSecondary(getComputedStyle(document.documentElement).getPropertyValue('--secondary-color'));
    } else {
      setPrimary(getComputedStyle(document.documentElement).getPropertyValue('--secondary-color'));
      setSecondary(getComputedStyle(document.documentElement).getPropertyValue('--primary-color'));
    }
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="1900" height="930" viewBox="0 0 1900 930" fill="none" className='fixed z-[-1] w-min h-screen top-0 right-0 '>
      ...
      <defs>
        ...
        <linearGradient id="paint0_linear_1020_42508" x1="937.274" y1="1846.28" x2="-174.901" y2="1240.11" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="0.800000" stopColor={secondary} />
        </linearGradient>
      </defs>
    </svg>
  )
