...
export const Tutorial = () => {
  ...
  const { isTutorialMade, setStep: setStepContext } =
  ...
  const handleJoyrideCallback = (data: CallBackProps) => {
    ...
    if (data.index > 0) {
      const oldTarget = document.querySelector(
        steps.steps[data.index - 1].target as string
      );
      if (data.index > 1) {
        const moreOldTarget = document.querySelector(
          steps.steps[data.index - 2].target as string
        );
        moreOldTarget?.classList.remove("auto-events");
        moreOldTarget?.classList.add("none-events");
      }
      oldTarget?.classList.add("auto-events");
    }
        ...
        options: {
          primaryColor:
            theme == "light"
              ...
              : "var(--secondary-color)",
          overlayColor:
            theme == "light" ? "rgba(0, 0, 0, 0.3)" : "rgba(0, 0, 0, 0.4)",
